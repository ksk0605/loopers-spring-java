...
-- 1,000,000개 상품에 대한 이미지 데이터 삽입 (상품당 1~5개 이미지)
...
WITH RECURSIVE image_data AS (
    ...
    FROM image_data
    WHERE product_id_tmp < 1000000 -- 100만 개로 변경
)
SELECT
    NOW(),
    NOW(),
...
-- 1,000,000개 상품에 대한 옵션 데이터 삽입 (상품당 1~3개 옵션)
...
SELECT
    ...
    product_id_tmp,
    CONCAT('Color'),
    CONCAT('Value ', option_idx),
    CAST(RAND() * 5000 + 100 AS DECIMAL(10, 2))
FROM option_data;

-- 총 15,000,000개의 좋아요 데이터 삽입 (100만개 상품 + 100개 브랜드에 평균 15개씩)
...
WITH RECURSIVE like_data AS (
    ...
    FROM like_data
    WHERE id < 15001500 -- 100만개 상품에 평균 15개씩 + 100개 브랜드 * 15개
)
SELECT
    user_id_tmp,
    -- 100만개 상품 또는 100개 브랜드 중 무작위 선택
    CASE WHEN RAND() > 0.9999 THEN CAST(RAND() * 99 + 1 AS UNSIGNED) ELSE CAST(RAND() * 999999 + 1 AS UNSIGNED) END AS target_id_tmp,
    CASE WHEN RAND() > 0.9999 THEN 'BRAND' ELSE 'PRODUCT' END AS target_type_tmp
FROM like_data;
